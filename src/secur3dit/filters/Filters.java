...
public class Filters {
    ...
    /**
     * @param angle: floating-point degree, anti-clockwise Produces a clipped image
     *               rotated by angle degrees, by treating the image as a rectangle
     *               on a cartesian plane and using bilinear interpolation to find
     *               the pixel value at every co-ordinate that will not have a black
     *               color after rotation. For more information about the math
     *               involved, visit
    ...
    public static BufferedImage rotate(BufferedImage image, double angle) {
        ...
        for (int i = 0; i < height; ++i) {
            for (int j = 0; j < width; ++j) {
                ...
                if (Helpers.outOfBounds(floored[0], width) || Helpers.outOfBounds(floored[1], height)
                        || Helpers.outOfBounds(ceiled[0], width) || Helpers.outOfBounds(ceiled[1], height)) {
                    continue;
                }
                ...
                int finalColor = Helpers.bilinearInterpolation(topLeft, topRight, bottomLeft, bottomRight, deltaX,
                        deltaY);
            ...
            for (int j = 0; j < width; ++j) {
                ...
                for (int m = 0; m < 3; ++m) {
                    for (int n = 0; n < 3; ++n) {
                        ...
                        if (Helpers.outOfBounds(indexI, height) || Helpers.outOfBounds(indexJ, width)) {
                            continue;
                        }
                ...
                for (int k = 0; k < 3; ++k) {
                    sobelXY[k] = (int) Math
                            .round(Math.sqrt((sobelSumX[k] * sobelSumX[k] + sobelSumY[k] * sobelSumY[k])));
